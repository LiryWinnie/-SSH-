...
<body>
   ...
   <div class="info">
       <s:form action="pubquestion_second" method="post" namespace="/">
           <s:actionerror/>
               <p>考试科目：${sessionScope.e.getCourse()}</p>
               <p>考试名称：${sessionScope.e.getEname()}</p>
               <p>考试时间：${sessionScope.e.getTime()}</p>
               <a>之前：    易：${sessionScope.b.getEasy()}</a>
               <a> 中：${sessionScope.b.getMiddle()}</a>
               <a> 难：${sessionScope.b.getDifficult()}</a>
               <p>院长评论：  ${sessionScope.b.getComment()}</p>
